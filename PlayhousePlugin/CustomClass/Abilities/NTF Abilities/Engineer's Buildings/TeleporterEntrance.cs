...
{
    public class TeleporterEntrance : CooldownAbilityBase
    {
        ...
        public TeleporterEntrance(Player ply)
        {
            Ply = ply;
        }
        ...
        private IEnumerator<float> BuildTeleporterEntrance()
		{
			...
			BuildingMapObject = UtilityMethods.SpawnSchematic("TeleporterBlueStatic", buildingPosition);
			IsBuilt = true;
			IsSpinning = false;
			
			while (Ply.CustomClassManager().CustomClass?.Name == "NTF Engineer")
			{
				yield return Timing.WaitForSeconds(1f);
				var teleporterExit = ((TeleporterExit) Ply.CustomClassManager().CustomClass.ActiveAbilities[3]);
				...
				if (teleporterExit.IsBuilt && !IsSpinning)
				{
					...
					BuildingMapObject.Destroy();
					BuildingMapObject = UtilityMethods.SpawnSchematic("TeleporterBlueSpinning", buildingPosition);
					IsSpinning = true;
					continue;
				}
				...
				{
					if (ply.Role.Team != Team.MTF && ply.Role.Team != Team.RSC && !ply.IsCuffed) continue;
					...
					if (teleporterPlayers.ContainsKey(ply))
					{
						...
						ply.Position =
							((TeleporterExit) Ply.CustomClassManager().CustomClass.ActiveAbilities[3])
