...
{
    public class NTFGasGrenade : NonCooldownAbilityBase
    {
        public override string Name { get; } = "Gas Grenade";
        public override Player Ply { get; }
        public int GasAmount = 0;

        public NTFGasGrenade(Player ply)
        {
            Ply = ply;
        ...
        private IEnumerator<float> GenerateGas()
        {
            ...
            while (Ply.CustomClassManager().CustomClass?.Name == "NTF Exterminator")
            {
                yield return Timing.WaitForSeconds(1);
                if(GasAmount != 60)
                    GasAmount += 1;
        ...
        {
            var gasGrenade = MapUtils.GetSchematicDataByName("GasGrenadeBlue");
            var gasGrenadeObject = ObjectSpawner.SpawnSchematic("GasGrenadeBlue",
            ...
            while (time != 30)
            {
                ...
                {
                    if ((ply.Role.Team == Team.CHI || ply.Role.Team == Team.CDP || ply.Role.Team == Team.SCP) && !ply.IsCuffed)
                    {
