        ...
        */
        ...
        private IEnumerator<float> Teleporter()
		{
			...
			while (Ply.CustomClassManager().CustomClass?.Name == "Machinist")
			{
				...
				{
					if (ply.Role.Team != Team.CHI && ply.Role.Team != Team.CDP) continue;
					...
					if (teleporterPlayers.ContainsKey(ply))
					{
						...
						{
							ply.ShowCenterDownHint($"<color=#007808>You will be teleported to: <color=#00d90e>{RoomName[teleporterPlayers[ply].Room.Type]}</color></color>", 3);
						...
						if (ply.Zone == ZoneType.Surface)
						{
							...
							teleporterPlayers.Add(ply,
								new TeleporterPlayers(1,
									true,
									Rooms.PickRandom()));
							ply.ShowCenterDownHint($"<color=#007808>Teleporter charging, stand still!</color>", 3);	
						}
						else
						{
