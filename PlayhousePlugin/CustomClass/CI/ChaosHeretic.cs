...
{
    public class ChaosHeretic : CustomClassBase
    {
        public override Player Ply { get; set; }
        public override string Name { get; } = "Heretic";
        ...
        public ChaosHeretic(Player ply)
        {
            ...
            {
                UtilityMethods.GiveCustomPlayerInfo(ply, $"{SillySundayNames.ChaosHeretic.PickRandom()}\n(Custom Class)", -1, "army_green");
            }
            else
            {
                UtilityMethods.GiveCustomPlayerInfo(ply, "Heretic\n(Custom Class)", -1, "army_green");
            }


            ply.Broadcast(10, "<size=40><b><i>You have spawned as a <color=green>Heretic</color>\nThis is a custom class with special abilities. To learn how to use them, press your console key (~ by default) or join our Discord at discord.gg/kognity</i></b></size>");
            ply.SendConsoleMessage("Name: Chaos Heretic\n\nDescription: Gain temporary strength but suffer the increasing burden of withdrawal.\n\nAbility1: Tonic Shot\n\nTo get the binding commands for the abilities type \".commands\"", "yellow");
        }
    }
}